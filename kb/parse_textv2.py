...
for title in tqdm.tqdm(db.titles()):
    unique = set()
    ...
    for p_id, paragraph in enumerate(paragraphs):
        ...
        for anchor in wiki_links:
            ...
            text_with_entity = text_with_entity[:char_start] + f'<e> ' + entity + f' </e>' + text_with_entity[char_end:]

        sentences = []
        paragraphs_acc.append(text_with_entity)
        try:
        ...
        for _, sentence in enumerate(sentences):
            ...
            example = {"title": title, "p_id": p_id, "s_id": s_id, "sentence": sentence, "paragraph": len(paragraphs_acc)-1}
...
pickle.dump(paragraphs_acc, open(f"dumps/{args.lan}_{args.paragraph}_paragraphs.pickle", "wb"))
print(f"total p: {p_id_acc}")
print(f"total s: {s_id_acc}")