...
parser.add_argument(
    '--conll_folder',
    help='The data folder you want to generate contexts, the code will read train, dev, test data in the folder in conll formatting.')
parser.add_argument(
    '--lang',
    help='The language code of the data, for example "en". We have specical processing for Chinese ("zh") and Code-mixed ("mix").')
parser.add_argument(
    '--use_sentence',
    action='store_true',
...
def gen_chinese_search_query(sentences):
    ...
    for sentence in sentences:
        ...
        before_word_is_zh = len(
            re.findall(
                r'[\u4e00-\u9fff]+',
                sentence[0].split()[0])) > 0
...
#    all_special_spans = []
    try:
        for m in p.finditer(paragraph):
            start, end, _ = m.start(), m.end(), m.group()
            span_len = end - start
...
def process_google(
        ...
        min_rank=0,
        lang=None,
        is_image_retrieval=False,
        image_ids=None,
        is_windowed_entity=False,
        window_size=0,
        is_wiki_retrieval=False,
        space_zh=False,
        is_test=False):
    new_sentences = []
    max_idx = 999
    ...
    if is_conll:
        eos = '<EOS> B-X B-X B-X'
        eos = '<SEP>	O	O	O	O	O	O	_	_	 StartOfContext'
    else:
        # eos = '<EOS> B-X'
        eos = 'O'
    if for_luke:
...
#        import pdb;pdb.set_trace()
        ...
        # if keyword in google_dict:
            ...
            # import pdb;pdb.set_trace()
        success = True
        if (lang != 'zh' and lang != 'multi' and not is_windowed_entity and keyword.replace(' ', '') not in google_dict) or (
            lang == 'zh' and temp_keyword.replace(' ', '') not in google_dict and keyword.replace(' ', '') not in google_dict) or (
            is_windowed_entity and not success) or (
                lang == 'multi' and temp_keyword.replace(' ', '') not in google_dict and \
                    temp_keyword2.replace(' ', '') not in google_dict and keyword.replace(' ', '') not in google_dict):
            if for_luke:
            ...
            if lang is not None:
                ...
                if old_lang is not None:
                    ...
                    failed_dict.append(
                        lang + '\t' + lang + '_' + str(sent_id) + '\t' + keyword)
                failed_id += 1
            else:
...
#        pdb.set_trace()
        if is_windowed_entity:
            ...
            try:
                context = google_dict[temp_keyword.replace(' ', '')].copy()
            except BaseException:
                context = google_dict[keyword.replace(' ', '')].copy()
        elif lang == 'multi':
            try:
                context = google_dict[temp_keyword.replace(' ', '')].copy()
            except BaseException:
                if temp_keyword2 not in google_dict:
                    ...
                    context = google_dict[temp_keyword2.replace(' ', '')].copy()
        else:
            context = google_dict[keyword.replace(' ', '')].copy()
        ...
        for cxt_id, cxt in enumerate(context):#[:4]):
            ...
            if len(split_seq) > 1:
                if cxt_id > 1:
                    split_seq = ['<SEP>'] + split_seq
            
            if for_luke:
                if is_conll:
                    words = [word + '	O	O	O	O	O	O	_	_	_' for word in split_seq]
                else:
                    words = [word + '	O	O	O	O	O	O	_	_	_' for word in split_seq]
            else:
                if is_conll:
...
#            pdb.set_trace()
#            words = ['[SEP]' + '	O	O	O	O	O	O	_	_	_' ] + words
            ...
            if len(words) + len(start_word) + len(end_word) + \
                    len(sentence) + add_eos > length_limit:
                break
...
def get_named_entity(sentence, is_bio=True, is_conll=True):
    entity_dict = {}
    named_entity = []
    correspond_label = []
    for word in sentence:
        ...
        if label.startswith('B-'):
            if len(named_entity) != 0:
                entity_dict[' '.join(named_entity)] = correspond_label
                named_entity = []
                correspond_label = []
...
def unlabeled_assignment(sentences, google_dict, failed_dict):
    ...
    for sentence in sentences:
        ...
        context = google_dict[keyword.replace(' ', '')]
        start_word = []
        end_word = []
...
def filter_context_sentences(sentences):
    ...
    for sentence in sentences:
        ...
        for token in sentence:
            ...
            if 'O' in token and '<EOS>' in token:
                flag = 1
                break
        if flag:
            new_sentences.append(sentence)
...
def add_to_dict(
    ...
    if flag:
        return google_dict, wrong_count, chunked_count
    if keyword.replace(' ', '') not in google_dict:
        google_dict[keyword.replace(' ', '')] = set()
    if has_rank:
        # google_dict[keyword].add((int(rank), desc))
        google_dict[keyword.replace(' ', '')].add((rank, desc))
    else:
        google_dict[keyword.replace(' ', '')].add(desc)
    if lang_dict is not None and lang is not None:
        lang_dict[keyword.replace(' ', '')] = lang
    return google_dict, wrong_count, chunked_count
...
#        title = "[ " + title + " ]"
        ...
        if use_sentence:
            ...
            desc = title + ' ' + desc
        rank += 1
    elif len(res) == 4 and is_wiki_retrieval:
        sentence, desc, title, score = res
#        title = "[ " + title + " ]"
        link = 'None'
        language = 'unk'
        if use_paragraph_entity:
...
to_write, train_sentences = count_file(
    args.conll_folder + '/HIPE-2022-v2.1-'+dataset+'-train-' + args.lang + '.tsv', 'train', is_file=True, comment_label='# id')
to_write, dev_sentences = count_file(
    args.conll_folder + '/HIPE-2022-v2.1-'+dataset+'-dev-'+ args.lang +'.tsv', 'train', is_file=True, comment_label='# id')
to_write, test_sentences = count_file(
    args.conll_folder + '/HIPE-2022-v2.1-'+dataset+'-test-ELmasked-'+ args.lang +'.tsv', 'train', is_file=True, comment_label='# id')
to_write, test_masked_sentences = count_file(
    args.conll_folder + '/HIPE-2022-v2.1-'+dataset+'-test-allmasked-'+ args.lang +'.tsv', 'train', is_file=True, comment_label='# id')
...
print('\nTRAIN')
new_train_sentences, failed_dict = process_google(train_sentences, google_dict, failed_dict, is_conll=is_conll, 
                                                  clean_file=False, full_doc=True,
                                                  add_eos='eos' in dataset_name, length_limit=length_limit, 
                                                  for_luke='luke' in dataset_name, lang=args.lang, 
                                                  is_wiki_retrieval=is_wiki_retrieval)
print('\nDEV')
new_dev_sentences, failed_dict = process_google(dev_sentences, google_dict, failed_dict, is_conll=is_conll, 
                                                clean_file=False, full_doc=True,
                                                add_eos='eos' in dataset_name, length_limit=length_limit, 
                                                for_luke='luke' in dataset_name, lang=args.lang, is_wiki_retrieval=is_wiki_retrieval)
print('\nTEST')
new_test_sentences, failed_dict = process_google(test_sentences, google_dict, failed_dict, is_conll=is_conll, 
                                                 clean_file=False, full_doc=True,
                                                 add_eos='eos' in dataset_name, length_limit=length_limit, 
                                                 for_luke='luke' in dataset_name, lang=args.lang, is_wiki_retrieval=is_wiki_retrieval)
print(len(new_test_sentences), len(test_sentences))
print('\nTEST MASKED')
new_masked_test_sentences, failed_dict = process_google(test_masked_sentences, google_dict, failed_dict, is_conll=is_conll, 
                                                 clean_file=False, full_doc=True,
                                                 add_eos='eos' in dataset_name, length_limit=length_limit, 
                                                 for_luke='luke' in dataset_name, lang=args.lang, is_wiki_retrieval=is_wiki_retrieval)
...
dataset_name = 'datasets/'+dataset+'/'+ args.lang +'/hipe'

#import pdb;pdb.set_trace()
write_dir = dataset_name + '_doc_full_wiki_v' + str(args.version) + suffix
if not os.path.exists(write_dir):
    os.mkdir(write_dir)
    
    print(write_dir)

write_file(write_dir + '/train_all.txt', new_train_sentences + new_dev_sentences, max_len=length_limit)
...
write_file(write_dir + '/test_masked.txt', new_masked_test_sentences, max_len=999)
all_sentences.append(new_test_sentences)
all_sentences.append(new_train_sentences)


# Generate for all the languages
# failed_diwrite_file(write_dir + '/test.txt', new_test_sentences, max_len=999)
#ct =[]
# mis_seped_sentences = []
# length_limit = 510
# dataset_names= {'DE-German':'de', 'ES-Spanish':'es', 'NL-Dutch':'nl', 'RU-Russian':'ru','ZH-Chinese':'zh','KO-Korean':'ko', 'MIX_Code_mixed':'mix', 'TR-Turkish':'tr', 'HI-Hindi':'hi','FA-Farsi':'fa','EN-English':'en','BN-Bangla':'bn'}
