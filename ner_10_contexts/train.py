...
from embeddings import RobertaEmbedding, BertEmbedding, CamembertEmbedding, XLMRobertaEmbedding
import os
import torch
import argparse
...
parser.add_argument('--dataset', type=str, default='newseye-v2',
                    choices=['newseye-v2'])
...
VOCAB = data_bundle.get_vocab('target').idx2word
...
def predict(path, data_bundle, predictor, predict_on='test', do_eval=False):
    ...
    with open(path.replace(dataset_dir, output_dir), 'w') as f:
        print(path.replace(dataset_dir, output_dir))
        ...
        for i, j in zip(dataset_test, predictions['pred']):
            if type(j[0]) == int:
                ...
                labels = list([VOCAB[x] for x in j[0]])
...
def main():
    ...
    if args.model == 'bert':
        ...
        model = StackedTransformersCRF(tag_vocabs=[data_bundle.get_vocab('target')], embed=embed, num_layers=num_layers,
    ...
    callbacks.extend([clip_callback, evaluate_callback, checkpoint_callback])
    ...
    if not args.do_eval:
        ...
        predict(os.path.join(directory, 'predictions_dev.txt'),
                data_bundle, predictor, 'dev')
        predict(os.path.join(directory, 'predictions_test.txt'),
