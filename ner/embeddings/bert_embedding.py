...
#
#        word_piece_dict = {'[CLS]': 1,ong them, {new_add_to_bpe_vocab} added to the BPE vocab.")
...
#        import pdb;pdb.set_trace()
        self._cls_index = self.tokenizer.cls_token_id #self.tokenizer.vocab['[CLS]']
        if self._cls_index is None:
            self._cls_index = self.tokenizer.bos_token_id
            
        self._sep_index = self.tokenizer.sep_token_id #self.tokenizer.vocab['[SEP]']
        if self._sep_index is None:
            self._sep_index = self.tokenizer.eos_token_id

        self._word_pad_index = vocab.padding_idx
        self._wordpiece_pad_index = self.tokenizer.pad_token_id #self.tokenizer.vocab['[PAD]']
